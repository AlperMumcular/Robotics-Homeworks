...
while True:   
    ...
    komo.addObjective([2], ry.FS.positionDiff, ['new_base_coordinates', 'new_cargo_coordinates'], ry.OT.eq, [2e1])    
    ...
    for pos in path:
        cargo_loc = C.frame('cargo').getPosition()
        speed = (math.sqrt((cargo_loc[0]-pos[0])**2 +(cargo_loc[1]-pos[1])**2 +(cargo_loc[2]-pos[2])**2 )) *speed_coef
        bot.moveTo(pos, speed)
